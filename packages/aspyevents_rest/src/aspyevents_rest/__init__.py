...
from aspyevents_rest.domain.config.rest_server_config import AspyEventsRestAppConfig

logger = structlog.get_logger(__name__)

_notifications_rest_app_config: Optional[AspyEventsRestAppConfig] = None
...
def get_notifications_rest_config() -> AspyEventsRestAppConfig:
    global _notifications_rest_app_config

    if _notifications_rest_app_config is None:
        config = get_config()
        model = config.to_pydantic(AspyEventsRestAppConfig)
