...
class EventsClientFactory(
    TypedPluginFactory[IEventsClientPort, EventClientConfig]
):
...
def create_events_client(
    ...
    logger.debug(
        "Creating event store repository from config", config=config
    )
    factory = EventsClientFactory(resolver=resolver)
    client = factory.create(config)
    logger.debug(
        "event store adapter created", client_type=type(client).__name__
    )
    return client
